...
A **spectrum of six espresso recipes** produced on the *same* stock 9-bar machine by holding **dose, temperature, coffee, and pressure constant** and varying only two levers — **yield (brew ratio)** and **grind**. From most concentrated to most dilute: **Ristretto → Normale → Lungo → Turbo → Allongé → Sprover** (café crème). They are "all roughly espresso, just different iterations" (Hendrick).
...
## The spectrum (illustrative single pulls — [[brewing/sources/every-espresso-recipe-explained-hendrick]])
...
*Bands are Hendrick's stated ranges; EY figures are single measured pulls — treat as illustrative.*
...
- **Turbo debunks the 6-bar myth.** A 7–15 s shot physically cannot build 6 bar (Hendrick's hit ~5) — so **any** machine, even a 20-bar pump, produces a turbo. No OPV mod required. See the [[brewing/entities/christopher-hendon|Hendon]] & Cameron paper origin.
...
- **EY dogma is optional.** Hendrick often targets **15–18% EY** (VOC preservation via fast shots), not the 18–22% convention; **ratio is the biggest single dictator** of EY.

## Relationship to the Rao/Decent framework

Hendrick's spectrum is **stock-machine-first**; Rao's ([[brewing/concepts/espresso-fundamentals]]) is **Decent/flow-control-first**. They agree on the physics (coarse + high flow favors light roasts; cool the big-yield styles) but differ on numbers and reach:

- **Allongé:** Rao 26–27% EY at 4.5 ml/s (Decent); Hendrick 22.25% at 1:5.6 on 9-bar. "Allongé" spans a wide EY depending on flow control and grind.
...
## Key Sources
- [[brewing/sources/every-espresso-recipe-explained-hendrick]] — the six-style spectrum with measured grind/dose/yield/TDS/EY; grind-ceiling logic; turbo 6-bar-myth debunk; temperature × yield; high-medium (practitioner/educator)
...
- [[brewing/concepts/bypass]] — Hendrick's preferred alternative to lungo (dilute a ristretto)
...
- Reconcile the allongé EY gap (Rao 26–27% vs Hendrick 22%) — is it flow control, grind, coffee, or measurement?
